...
- **El motor no está acoplado a ningún corpus LATAM.** El único `RuleBase` cargado (`jp-civil-612-sublease-demo`, en `src/reasoning/rulesets/jp_civil_612_sublease.py`) codifica el ejemplo del propio paper — Art. 612 del Código Civil japonés — elegido deliberadamente por ser citable y verificable contra una traza de ejecución publicada, no contenido latinoamericano. `Rule.source_uid` existe como campo de enlace hacia `ArticleRecord.uid` (`src/models.py`), pero no está poblado: ninguno de los 8 corpus registrados en `config/corpus_registry.yaml` tiene todavía anotación N2/N3 real (ver §5 anterior y `docs/notas_gobernanza.md`) de la que derivar reglas jurídicas ejecutables. `ExceptionInfo`/`PresumptionInfo.burden_shifts_to` (`src/models.py:159-176`) siguen siendo anotación descriptiva por artículo, no reglas del motor — el puente entre ambas capas (traducir automáticamente un `NormativeStatement` anotado en un `Rule`/`ExceptionLink` del motor) no está construido.
- **La extracción de reglas desde texto normativo no es automática.** El ruleset del paper se escribió a mano, no se derivó de `src/pipeline.py`. Aplicar este motor a un artículo real de cualquiera de los 8 códigos exigiría, hoy, codificar sus reglas y excepciones manualmente — el mismo trabajo de anotación jurídica que `docs/informe_hito1.md` y `docs/notas_gobernanza.md` ya señalan como pendiente para N2/N3.

En síntesis: el salto entre "hay una taxonomía que anota excepciones y cargas de la prueba por artículo" y "hay un motor que razona sobre esas anotaciones" ya no es una promesa incumplida — el motor existe, funciona, y está probado contra su propia fuente académica. Lo que falta es el siguiente salto: conectarlo con contenido jurídico latinoamericano real, lo cual depende de que exista anotación N2/N3 verificada de la que no fabricar reglas.
